    ...
    def train(self, X, Y):
        for i in range(self.K):
            for j in range(i):
                ...
                Xij = X[(Y == i) | (Y == j), :]
                Yij = (Y[(Y == i) | (Y == j)] == j) * \
                    2 - 1  # +1 if it's j, -1 if it's i
                try:
                    self.f[i][j].fit(Xij, Yij)  # For sklearn implementations
                except:
                    # For implementations of binary.py
...
class TreeNode:
    ...
    def getLabel(self):
        if self.isLeaf:
            ...
            raise Exception("called getLabel on an internal node!")

    def getLeft(self):
        if self.isLeaf:
            raise Exception("called getLeft on a leaf!")
        else:
            return self.left

    def getRight(self):
        if self.isLeaf:
            raise Exception("called getRight on a leaf!")
        else:
            return self.right
